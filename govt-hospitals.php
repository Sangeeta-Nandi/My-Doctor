...
?>
	...
	<!-- Sub Page Heading Area Starts Here -->
		<div class="sub-page-heading-area">
			<div class="featured-single-item bdgovt">
				<img src="img/bdgovt.png" alt="Government Hospital" />
				<h3>Govt. Hospital</h3>
			</div>
	...
	$query="Select gov_hs_name,gov_hs_website from govt_hospital";
	
	$exquery=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($exquery))
	{
		
	    $diagnosticsName=$row['gov_hs_name'];
		
		$gov_hs_website=$row['gov_hs_website'];
	
	
	?>
			<div class="result-single-item-box">
				<div class="govt-hospital-result-single-item">
					<h4><a href="<?php  echo $gov_hs_website; ?>" target="_blank"><?php echo $diagnosticsName;  ?></a></h4>
				</div>
			</div>
